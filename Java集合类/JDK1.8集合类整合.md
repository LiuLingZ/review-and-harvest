...
如果key是null，则去索引位0的桶获取
key非null，进行定位
如果桶是空的，就直接赋值
如果桶不是空，就开始按照链表的形式循环比对，如果key相同，覆盖，一直遍历到末尾
此时需要创建一个新的Entry，那么插入前，需要判断是否需要resize
如果resize，	那么就需要重新rehash这个Entry，然后再最新定位的地方put

**get：**
key为null，就去第0个桶获取
否，定位，遍历，直到找到返回；找不到，返回null
...
- ConcurrentHashMap的size（）采用乐观锁的实现。即统计长度时，先从每个Segment统计Entry个数，最后合并。期间还会统计所有Segment修改次数，如果和统计数目前的修改次数一致，则统计成功，如果不一致，重新统计（因为统计过程有其他线程并发增加、删除了元素）。当失败次数达到一定次数，就直接锁住全部Segment，再重新统计。
  ...
  定位到Segment，每个Segment也是一个HashMap
  然后尝试获取锁，Segment继承ReentrantLock,尝试获取锁，失败自旋，最多64次后，改成阻塞获取锁
  然后通过hash定位到桶，无则直接创建Entry赋值，有就遍历比对，要么覆盖要么新创建一个
  如果触发扩容，就扩容再插入
  然后释放锁
...
JDK1.8 ：
get()
	定位hash，判断第一个节点是否是要找的
	不是，判断是否为树节点，树节点就用getTreeNode方法找
	不是，就循环遍历链表 key相等(同时也判断hash是否相等)后返回
	找不到就返回null



put()
	hash表是否为空，为空初始化（resize）
	不为空，定位hash到index
	看这个index对应的是否为null，如果是，直接put ，之后判断是否要扩容
	如果不是，第一个桶是否相同，是就覆盖
	第一个桶不相同，判断树节点，是的话按树的方式插入
	不是树类型，就链表遍历，覆盖，或者新增，新增之后，判断此时是否已经有7个节点（实际是大于7，只是计数还没有算进去），并且hash容量已经到了64了，如果是，就转为红黑树
	如果大于7而容量小于64，就resize（此时链表长度=8了）
	插入完成之后，最后再判断此时节点数>扩容阈值，是就resize扩容，结束。



resize()
JDK1.7的扩容中，元素重新定位是直接重新计算index,然后原先相同位置的链表在新链表位置仍然相同时，会倒置 。

JDK1.8
	不是重新计算index，而是通过 e.hash & oldCapacity ，如果为0，则在新数组index不变，如果为1，则
	新位置 = 原下标 + 原数组长度。
	
	比如 
	oldCapacity = 16   → 0001 0000
    e.hash = 15 → 0000 1111 
    相与 => 0 , 所以在新的32长度的数组中位置不变
    
    而
    e.hash = 17 = 0001 0001 
    e.hash & 16 = 1
    那么新位置 = 1 + oldCapacity = 17 
...
- 转红黑树过程：
  ...
  链表=8，并且容量>=64 ,否则链表=8也是触发hashMap扩容的
...
链接：https://www.jianshu.com/p/81d848ea6c1a （好的）

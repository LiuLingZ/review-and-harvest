...
fork一份子进程并不会立刻拷贝一个一模一样大小的内存出来。rdb(快照式持久化)是通过COW ， copy on write方式的，内存多是分为一个个内存页的，持久化时一个一个内存页拷贝，而被父进程用到的内存页，父进程会拷贝一个一模一样的内存页，在新的内存页上面操作（COW机制），然后子进程就可以使用原来的内存页进行持久化，互不干扰，最终持久化完毕，代替新的 rdb文件，所以最不济需要2倍内存大小，但不是都需要2倍内存大小。
...
即 rdb + aof 方式，可以看到，原先一大部分是使用了rdb的，后面有新数据变化时，采用 aof ，而此时的aof已经不是完整的aof文件，很小，重写的效率提高了很多。

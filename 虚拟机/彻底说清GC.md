...
​	Region实现并不简单，因为存在这么个问题：对象的引用极可能不是在同一个Region中分配的，那么如何避免全堆扫描去找那个引用的对象呢？就用到了 Remembered Set这个数据结构，每个Region内含一个。当一个对象进行引用的写的时候，判断是否在同一个Region中，如果不是，就把对应的region写到Remember Set中。 这样，当要回收的时候，直接扫描Remembered中。
